...
- **Inclination is actively targeted** (ADR 0024, closes audit AD-17). The launch azimuth uses the Earth-rotation-corrected *flight* azimuth (`atan2(v·sin Az_in − ωR cos lat, v·cos Az_in)`), and the S2 terminal phase yaw-steers to null the inertial out-of-plane velocity relative to the target orbital plane (`GUIDANCE_PLANE_STEER_GAIN`, clamped to `GUIDANCE_MAX_YAW_DEG`). The achieved inclination is ~51.0° (target 51.6°; the ~0.5° residual is a 74 m/s on-orbit correction, down from ~995 m/s with the old uncorrected azimuth). The steering is exo-atmospheric (terminal phase only), so peak loads are unchanged, and it no longer regresses Monte-Carlo robustness (24/24) — the AD-19 blocker (PEG terminal phase riding the 25° FTS limit) was removed by the N-01 work (ADR 0021/0023).
