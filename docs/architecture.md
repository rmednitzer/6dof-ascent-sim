...
- Ground-station range tracking (`GROUND_STATIONS`) at `GROUND_TRACK_UPDATE_HZ` (5 Hz per station), available whenever the vehicle is above a station's elevation mask — a nonlinear slant-range update (Jacobian = line-of-sight unit vector on position). Independent of GPS (the vehicle is tracked, not self-locating), so it bounds position covariance through the COCOM GPS-denied coast (ADR 0023)

Below the GPS ceiling attitude is observable through the specific-force/velocity coupling (`δv̇ = -[f_n]_x δθ`); above it the star tracker observes attitude directly (all three axes, including roll about the thrust axis, which the coupling cannot). The innovation gate rejects a measurement when its normalised innovation squared (NIS = `yᵀ S⁻¹ y`) exceeds the chi-square quantile at `EKF_INNOVATION_GATE_P` — a Mahalanobis test over the full innovation covariance, replacing the earlier per-component `|yᵢ| > kσᵢ` diagonal test. Non-finite innovations are rejected as a counted fault. Covariance update uses Joseph form. The `USE_ESTIMATED_ATTITUDE` flag (overridable; **default `True`**) selects whether guidance/control/FTS consume the estimated attitude (the closed-loop default) or the true attitude while the estimator runs in parallel.
