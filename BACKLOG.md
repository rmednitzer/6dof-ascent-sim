...
| N-01 | Revisit nav-uncertainty FTS aborts under IMU dispersion | medium | M | The paired Stage-2 Monte-Carlo campaign ([ADR 0020](docs/adr/0020-error-state-attitude-ekf.md)) shows ~33 % of dispersed runs FTS-abort, dominated by the `IMU_ACCEL_BIAS_MPS2` / `IMU_GYRO_BIAS_RADS` dispersions (drawn up to ~7× nominal) inflating the EKF position covariance past `FTS_COVARIANCE_LIMIT_M` (10 km) during the ~300 s GPS-denied upper-stage coast — **identical for the true- and estimated-attitude authorities**, so it is a pre-existing robustness property, not a Stage-2 regression. Options: tighten/justify the IMU-dispersion realism, revisit the 10 km FTS covariance limit, or model GPS reacquisition / additional aiding above the 60 km COCOM ceiling. | ADR 0020 | GNC eng |
