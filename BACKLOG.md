...
| [Q-06](audit/02-security-findings.md) | Remove dead `_v_p` in decay.py | info | S | **✅ Resolved:** `decay.py:231` computed vis-viva periapsis velocity, suppressed F841, never used it. The King-Hele formula was verified correct without it; the line is deleted. | — | Orbital eng |
| [Q-07](audit/02-security-findings.md) | Document the yaw sign convention | info | S | **✅ Resolved:** `control.py` now carries a one-line note on why the yaw PID output is negated relative to pitch (`torque_z = -L*Fy` vs `torque_y = +L*Fz` for the aft-mounted gimbal in `sim.main`), to prevent an accidental flip in future refactors. | — | GNC eng |
| Q-08 | Surface EKF-covariance proximity in Monte Carlo statistics | low | S | **✅ Resolved:** `sim/montecarlo/statistics.py::compute_statistics` computed `peak_ekf_uncertainty_m` into a `# noqa: F841`-suppressed, discarded variable instead of a `peak_ekf_pct` entry in `limit_proximity` (unlike `peak_q_pct`/`peak_g_pct`, which were reported). A Monte Carlo campaign therefore could not show how close dispersed runs came to the FTS covariance-abort limit — exactly the margin the N-01 ground-station-tracking work (ADR 0023) was driving toward zero aborts. Now computed and printed alongside the other two; test in `tests/test_fidelity_fixes.py::TestStatisticsPeakEkfProximity`. | — | Sim/QA eng |
...
| ID | Title | Sev | Effort | Rationale / approach | Depends on | Owner role |
|----|-------|-----|--------|----------------------|------------|------------|
| [S-05](audit/02-security-findings.md)b | Single-source the ruff version | low | S | CI and pre-commit pinned different ruff versions (realigned to 0.15.16 in the original fix, then again to 0.16.1 in #91). **Recurred a third time**: #93/#94 (Renovate) advanced `.pre-commit-config.yaml` to `v0.16.6` without touching the CI pin or the `pyproject.toml` dev floor, both still at `0.16.1`; realigned again (2026-09-12) to `0.16.6`. The underlying gap is still open: nothing derives CI's ruff from `.pre-commit-config.yaml`, so a bare-version bump of the pre-commit hook will drift the trio again. Prevent recurrence by deriving CI's ruff from `.pre-commit-config.yaml` (e.g. run `pre-commit run` in CI, or read the rev), so they cannot drift again. See [ADR 0005](docs/adr/0005-quality-tooling-strategy.md). | — | DevEx |
| T-02 | Make lint job match the support matrix | low | S | **✅ Resolved.** The premise (CI lints on 3.14 while the project supports only 3.11-3.13) no longer holds: `ci: add stable aggregate gate and Python 3.14 coverage` (#95) added 3.14 to the `classifiers` in `pyproject.toml`, the CONTRIBUTING.md matrix, and the `test` job matrix, so the lint/typecheck jobs' Python 3.14 is now inside the declared support matrix. | — | DevEx |
